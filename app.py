...
def assess_risk():
    try:
            ...
            try:
                ...
                if conn:
                    ...
                    if user_row:
                        ...
                        """, (
                            user_id,
                            response.get('app_db_overall_percent', 0),
                            response.get('app_db_overall_level', 'Unknown'),
                            response.get('app_db_dominant_category', 'Unknown'),
                            json.dumps(response.get('app_db_risk_breakdown', {})),
                            json.dumps(response)
                        ))
                        conn.commit()
        ...
        # Clean up helper keys before returning
        for key in list(response.keys()):
            if key.startswith('app_db_'):
                response.pop(key)
